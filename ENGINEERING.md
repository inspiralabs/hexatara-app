...
- **Dependency baru DISETUJUI eksplisit oleh Alif** (larangan #7 PRD.md,
  butuh izin eksplisit — ini izinnya, dicatat di sini): shadcn/ui + primitif
  UI-nya, `class-variance-authority`, `@tanstack/react-table` (data
  ...
  `lucide-react` (ikon). Command Palette (`cmdk`, ⌘J) TIDAK wajib — di
  §12.6.0 diputuskan **di-skip** (bukan fitur inti).
- **Koreksi primitif UI (2026-09-12, saat eksekusi §12.6.0):** ADR awal
  menulis "Radix UI primitives" karena itu yang umum disebut bersama
  shadcn/ui saat ADR ditulis. Di repo Hexatara, `components.json` sudah
  memakai style **`base-nova`** dengan primitif **`@base-ui/react`** (bukan
  `@radix-ui/*`). Keputusan: **tetap base-nova / Base UI** — setara secara
  peran (headless primitives di bawah komponen shadcn), sudah terpasang dan
  dipakai di puluhan komponen `src/components/ui/*`. Mengganti ke Radix
  klasik hanya demi literal ADR berarti rewrite fondasi tanpa nilai produk.
  Izin dependency ADR-019 tetap berlaku; yang berubah hanya nama paket
  primitif. `cmdk` tetap di-skip.
...
| 2026-09-12 | **ADR-019 dikoreksi saat §12.6.0:** primitif UI yang dipakai adalah shadcn style `base-nova` + `@base-ui/react` (sudah ada di repo), bukan Radix UI klasik yang tertulis di draf awal ADR. Alasan: setara peran, sudah terpasang; migrasi ke Radix = rewrite tanpa nilai. `cmdk` di-skip. |
| 2026-09-10 | **ADR-019 — Fase 12.6 dibuka, redesign total.** Alif memberikan referensi baru (studio-admin.arhamkhnz.com, shadcn/ui) dan minta seluruh komponen lama direplace. Dicek status Fase 12.5: hanya §12.5.1–§12.5.3 sudah jalan, §12.5.4 belum selesai, §12.5.5–§12.5.17 belum dimulai sama sekali — scope-nya diserap ke Fase 12.6 alih-alih dibangun dua kali (dengan design system lama lalu diganti lagi). §12.5.4–§12.5.17 di PANDUAN.md diarsipkan (ditandai, tidak dihapus). Dependency baru disetujui eksplisit: shadcn/ui, Radix UI (kemudian dikoreksi → Base UI / base-nova, lihat baris 2026-09-12), `@tanstack/react-table`, `next-themes`, Recharts, `lucide-react` (Command Palette/`cmdk` opsional). Warna mengikuti preset "Neutral" referensi untuk sementara (achromatic + merah untuk destructive), BUKAN token Hexatara — menunggu persetujuan Abi, dicatat sebagai blok paling akhir §12.6.14. Auth pakai varian v2 (form kiri, panel highlight kanan). Prinsip mobile-first ditegaskan ulang sebagai prioritas utama di setiap blok. Referensi struktural lengkap di `hexatara_ADMIN_DESIGN.md`, menggantikan `hexatara_DESIGN.md` untuk Fase 12.6 |
| 2026-09-12 | **Migrasi kode ADR-016 (XLSX) dituntaskan** — `leads-export.ts`/`penawaran-export.ts` diubah dari `papaparse`/CSV ke `xlsx` (dynamic import), label tombol jadi "Ekspor Excel". Dua template unduhan (`sertifikat-import-template`, `bank-soal-import-template`) dibuat ulang sebagai `.xlsx` asli, bukan `.csv`. `papaparse` **sengaja tidak dihapus** — `pnpm knip` mengonfirmasi masih dipakai `lib/import/spreadsheet.ts` untuk tetap menerima upload `.csv` lama (dual-format, F02.8/F03.13 sudah lolos dengan CSV). **Sonner toast** dipasang di setiap Server Action Admin (simpan/edit/hapus/reorder/impor/approve/tolak) lewat titik panggil client, tanpa mengubah signature 21 `actions.ts` — pesan gagal pakai `hasil.pesan` yang sudah ada, pesan sukses literal per aksi. Ditemukan sekaligus diperbaiki: `status-pengiriman-select.tsx` selama ini mengabaikan hasil gagal `updateStatusPengirimanAction()` sepenuhnya (tidak ada penanganan error sama sekali). **Reorder otomatis (ADR-014) diterapkan ke `quiz_questions`** — SQL usulan `docs/sql/21_reorder_quiz_questions.sql` (belum dijalankan), pola tombol naik/turun (bukan drag-and-drop, konsisten dengan `material_chapters`/kategori — ADR-014 di kode selalu begini walau teks ADR menyebut drag-and-drop), `soal-table.tsx` (DataTable) diganti `soal-list.tsx`. Field "Urutan" manual dihapus dari form soal; sekaligus diperbaiki bug lama di impor massal bank soal yang selalu memulai `urutan` dari 0 (bentrok dengan soal existing). **Task "Combobox untuk field pemilihan produk/batch" diaudit, tidak ada target** — satu-satunya FK-selector (kategori) sudah Combobox sejak F06.4; field CTA link (hero slide/popup) sengaja tetap teks bebas (ADR-011b) |
