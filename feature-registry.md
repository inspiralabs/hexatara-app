...
| F00.4 | next-intl + middleware + messages | WIP | `src/i18n/routing.ts`, `src/i18n/request.ts`, `src/i18n/navigation.ts`, `src/proxy.ts`, `next.config.ts`, `src/lib/i18n/pick.ts`, `messages/id.json`, `messages/en.json`; seluruh route `(public)`/`(auth)` dipindah ke `src/app/[locale]/` |  | |
...
| F01.11 | Pemilih bahasa di semua halaman publik | WIP | `src/components/language-switcher.tsx`, `src/i18n/navigation.ts` |  | |
