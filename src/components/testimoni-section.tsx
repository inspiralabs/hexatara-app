...
export async function TestimoniSection() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-16 md:py-24">
      <h2 className="text-xl font-bold text-foreground sm:text-2xl">{t("testimonialsHeading")}</h2>
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {data.map((testi) => (
          <figure
            key={testi.id}
            className="flex flex-col gap-3 rounded-xl border border-border bg-background p-4 shadow-float hover:-translate-y-0.5 hover:shadow-float-hover [transition:var(--transition-hover)]"
          >
            ...
            <figcaption className="mt-auto flex items-center gap-3">
              ...
              <div>
                ...
                {testi.peran_id && (
                  <p className="text-sm text-muted-foreground">{pick(testi.peran_id, testi.peran_en, locale)}</p>
                )}
              </div>
            </figcaption>
          </figure>
