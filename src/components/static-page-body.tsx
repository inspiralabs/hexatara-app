...
export async function StaticPageBody({ namespace }: { namespace: string }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="text-2xl font-bold text-warna-teks sm:text-3xl">{t("title")}</h1>
      <div className="mt-6 flex flex-col gap-4 text-base leading-relaxed text-warna-teks-2">
        {body.map((paragraf, i) => (
          <p key={i}>{paragraf}</p>
        ))}
      </div>
