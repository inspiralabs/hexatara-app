...
import { hapusLampiranAction } from './lampiran-actions';
...
export function LampiranRowActions({
  lampiranId,
  judul,
  onUbah,
}: {
  ...
  function konfirmasiHapus() {
    startTransition(async () => {
      const hasil = await hapusLampiranAction(lampiranId);
      if (!hasil.ok) {
  ...
  return (
    <>
      <DropdownMenu>
        <DropdownMenuTrigger render={<Button variant="ghost" size="icon" aria-label={`Aksi untuk lampiran ${judul}`} />}>
          <MoreVerticalIcon />
        </DropdownMenuTrigger>
      ...
      <AlertDialog open={hapusOpen} onOpenChange={setHapusOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus lampiran &quot;{judul}&quot;?</AlertDialogTitle>
