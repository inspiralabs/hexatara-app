...
import { hapusPopupAction } from './popup-actions';
...
export function PopupRowActions({
  ...
  function konfirmasiHapus() {
    startTransition(async () => {
      const hasil = await hapusPopupAction(popupId);
      if (!hasil.ok) {
        toast.error(hasil.pesan);
        return;
      }
      setHapusOpen(false);
      toast.success('Pop-up berhasil dihapus.');
      router.refresh();
    });
  }
  ...
  return (
    <>
      ...
      <AlertDialog open={hapusOpen} onOpenChange={setHapusOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus pop-up &quot;{judul}&quot;?</AlertDialogTitle>
