...
function runLighthouse(url, outJson) {
  return new Promise((resolve, reject) => {
    // Quote paths: Windows + spaces break unquoted --output-path
    const cmd = [
      'npx --yes lighthouse',
      JSON.stringify(url),
    ...
    const child = spawn(cmd, {
      ...
      env: {
        ...process.env,
        // Keep Chrome temp under project to avoid EPERM on AppData cleanup
        TMP: OUT_DIR,
        TEMP: OUT_DIR,
        TMPDIR: OUT_DIR,
      },
    ...
    child.on('close', (code) => {
      if (fs.existsSync(outJson) && fs.statSync(outJson).size > 1000) {
        // Windows often fails chrome-launcher destroyTmp (EPERM) after a good run
        resolve({ code, err });
        return;
      }
      reject(new Error(`lighthouse exit ${code}: ${err.slice(-800)}`));
    });
  });
}
...
function summarize(jsonPath, meta) {
  ...
  const opportunities = Object.values(audits)
    ...
    .map((a) => ({
      ...
      items: (a.details?.items || []).slice(0, 6).map((it) => ({
        url: it.url || null,
  ...
  for (const page of pages) {
    ...
    try {
      ...
      console.log(
        ...
        summary.performance,
        'LCP',
        summary.metrics.LCP && summary.metrics.LCP.displayValue,
        'final',
        summary.finalUrl
      );
    } catch (e) {
